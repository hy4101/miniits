package com.miniits.commons.query;

import com.miniits.commons.lang.SpringContext;
...
/**
 * Service基础类。此类基于Spring Data JPA进行封装（Spring Data JPA又是基于JPA封装，平台使用Hibernate作为JPA实现者）。
...
public class BaseJpaService<T, R> {
    ...
    public int getCount(String filters) throws ParseException {
        URLQueryParser queryParser = createQueryParser(filters);
    ...
    public void batchInsert(T[] array) {
        for (int i = 0; i < array.length; i++) {
            entityManager.persist(array[i]);
            if (i % 30 == 0) {
                entityManager.flush();
                entityManager.clear();
            }
