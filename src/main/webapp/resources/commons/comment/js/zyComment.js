        ...
        return this.each(function () {
            ...
            this.createAgoCommentHtml = function () {
                ...
                $.each(para.agoComment, function (k, v) {
                    ...
                    item += '	<a class="avatar">';
                    item += '		<img src=' + Util.getUrl() + '/resources/commons/images/foot.png>';
                    item += '	</a>';
                    item += '	<div class="content ' + topStyle + '">';
                    item += '		<a class="author"> ' + v.userName + ' </a>';
                    item += '		<div class="metadata">';
                    item += '			<span class="date"> ' + v.discussDate + ' </span>';
                    item += '		</div>';
                    item += '		<div class="text"> ' + v.discussText + ' </div>';
                    ...
                    if (v.pid == 0) {  // 不是
                        ...
                        // 判断父级评论下是不是已经有了子级评论
                        if ($("#comment" + v.pid).find(".comments").length == 0) {  // 没有
                            var comments = '';
                            comments += '<div id="comments' + v.pid + '" class="comments">';
                            comments += item;
                            comments += '</div>';

                            $("#comment" + v.pid).append(comments);
                        } else {  // 有
                            $("#comments" + v.pid).append(item);
                        }
                    }
                });
            ...
            this.createFormCommentHtml = function () {
                ...
                // 组织发表评论的form html代码
                var boxHtml = '';
                boxHtml += '<form id="replyBoxAri" class="ui reply form" style="margin-bottom: 60px;">';
                ...
                boxHtml += '		</div>';
                boxHtml += '		<div id="submitComment" class="ui button teal submit labeled icon" style="float: right;">';
            ...
            this.replyClickEvent = function () {
                ...
                $(self).find(".actions .reply").on("click", function () {
                    ...
                    $("#publicComment").on("click", function () {
                        var result = {
                            pid: fCode,
                            blogId: $("#inp_blog_id").val(),
                            discussText: $("#commentContent").val()
                        };
                        $.ajax({
                            url: Util.getUrl() + "/discuss/new_discuss",
                            data: {pid: fCode,blogId: $("#inp_blog_id").val(), discussText: $("#commentContent").val()},
                            dataType: "json",
            ...
            this.addFormEvent = function () {
                ...
                $("#submitComment").on("click", function () {
                    ...
                    $.ajax({
                        url: Util.getUrl() + "/discuss/new_discuss",
                        data: {pid: 0,blogId: $("#inp_blog_id").val(), discussText: $("#commentContent").val()},
                        dataType: "json",
                        success: function (data) {
                            if (data.successFlg) {
                                comment();
            ...
            this.addReplyCommentFrom = function (id) {
                ...
                $(self).find("#comment" + id).find(">.content").after(window.html());
            ...
            this.addNewComment = function (param) {
                ...
                item += '		<div class="metadata">';
                item += '			<span class="date"> ' + param.discussDate + ' </span>';
                item += '		</div>';
                item += '		<div class="text"> ' + param.discussText + ' </div>';
                item += '		<div class="actions">';
